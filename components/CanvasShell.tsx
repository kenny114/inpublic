...
export function CanvasTopBar({ title, saveState, onTitleChange, onExport, onDownloadLog }: { title: string; saveState: SaveState; onTitleChange: (title: string) => void; onExport: (kind: ExportKind) => void; onDownloadLog: () => void }) {
      ...
      <div className="canvas-topbar-actions">
        ...
        {ready && user ? (
          <DropdownMenu label="Account menu" trigger={<span className="canvas-account-trigger">{initial}</span>}>
            ...
            <button type="button" role="menuitem" onClick={signOut}><LogOut size={15} />Sign out</button>
          </DropdownMenu>
