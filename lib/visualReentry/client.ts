/**
 * Client-safe request wrapper for Visual Re-entry V1.1's decision call.
 *
 ...
 * resolve to `none` (Part 12 — a failure at any stage does nothing, never
 * retries). The `reason` distinguishes a request-level failure from a
 * parse failure so lib/visualReentry/orchestrate.ts can log the right
 * stage — see REASON_* in ./types.
 *
 * `signal` is the same AbortController idiom Board.tsx already uses for
 * Beat/Scribe/Story (aiAbortRef/scribeAbortRef/storyAbortRef). V1.1 reserves
 * aborts for reset/teardown; Board suppresses new requests while this one is
 * running instead of making continued speech invalidate useful work.
 */
